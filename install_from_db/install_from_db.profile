...
function install_from_db_form($form, &$form_state, &$install_state) {  
  ...
  $filename = $profile . '.mysql';
  $filename_demo = $profile . '_demo.mysql';

  $folder = DRUPAL_ROOT . '/profiles/' . $profile . '/db/';
  
  $conn = Database::getConnection('default');
  if ((!file_exists($folder.$filename) && !file_exists($folder.$filename_demo)) || !$conn || ($conn->driver() !== 'mysql')) {
  ...
  $install_state['parameters']['db_import_filename_demo'] = $filename_demo;
  ...
  $form['quickstart']['demo'] = array(
    '#type' => 'radio',
    '#value' => 'demo',
    '#return_value' => 'demo',
    '#title' => st('Demo installation'),
    '#description' => st('Install ERPAL with DEMO content and for your first tests.'),
    '#parents' => array('quickstart'),
  );
...
function install_from_db_form_submit($form, &$form_state) {
  ...
  $quickstart = ($form_state['input']['quickstart'] && $form_state['input']['quickstart'] != 'standard') ? $form_state['input']['quickstart'] : FALSE;
...
* re-import the users
*/
function _install_from_db_reimport_demo_users() {
  $is_quick_install = _erpal_is_quickinstall();
  if ($is_quick_install == 'demo') {
    $db_filename = DRUPAL_ROOT . '/profiles/erpal/db/erpal_demo_users.mysql';
  }
...
function install_from_db_install_profile_modules(&$install_state) {
  ...
  $is_quick_install = _erpal_is_quickinstall();
  if ($is_quick_install == 'demo') {
    $db_filename = DRUPAL_ROOT . '/profiles/' . $profile . '/db/' . $install_state['parameters']['db_import_filename_demo'];
  } elseif ($is_quick_install == 'quick') {
    $db_filename = DRUPAL_ROOT . '/profiles/' . $profile . '/db/' . $install_state['parameters']['db_import_filename'];
  }
